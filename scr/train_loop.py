...
import torch
def train(device, model, dataloader, optimizer, criterion, binary=True):
    model.train() 
...
def validate(device, model, dataloader, criterion, binary=True):
...
def train_for_epochs(device, epchs, model, trainloader, valloader, optimizer, criterion, binary=True):
    ...
    for epoch in loop:
        ...
        epoch_loss_train, epoch_accuracy_train = train(device, model, trainloader, optimizer, criterion, binary=binary)   

        history["train"]["loss"].append(epoch_loss_train)
        history["train"]["accuracy"].append(epoch_accuracy_train)

        epoch_loss_val, epoch_accuracy_val = validate(device, model, valloader, criterion, binary=binary)   
...
# the following loop does not look at validation data
    
def train_no_gt(device, model, dataloader, optimizer, criterion):
    model.train() 
...
def train_for_epochs_no_gt(device, epchs, model, trainloader, optimizer, criterion):
    history = {
        "loss": []
    }
    loop = trange(epchs)
    ...
    for epoch in loop:
        ...
        epoch_loss_train = train_no_gt(device, model, trainloader, optimizer, criterion)   
